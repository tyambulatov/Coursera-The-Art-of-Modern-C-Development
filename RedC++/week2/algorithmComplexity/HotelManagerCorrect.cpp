...
private:
    ...
    {
    public:
        void Book(const Booking& booking) // O(logQ)
        {
            last_bookings_.push(booking); // O(1)
            room_count_ += booking.room_count;
            ++client_booking_counts_[booking.client_id]; // какая сложность
            // количество записей booking от клиента logQ?
            // какое максимальное кол-во клиентов для одного отеля
        }
        int ComputeClientCount(int64_t current_time)
        {
            RemoveOldBookings(current_time); // logQ
    ...
    private:
        ...
        int room_count_ = 0;
        map<int, int> client_booking_counts_; // количество записей для клиента
