...
void EnsureNextSymbolAndRemove(basic_istream<char, char_traits<char>> &stream)
{
    if (stream.get() != '-')
    {
        throw domain_error("");
...
void IfPlusAddOne(basic_istream<char, char_traits<char>> &strStream, int &plusCounter)
{
    if (strStream.peek() == '+')
    {
        plusCounter += 1;
...
basic_istream<char> &operator>>(basic_istream<char> &stream, Date &date)
{
    ...
    stream.ignore(1);
    getline(stream, fullDateLine, ' '); //возможно нам нужен перенос строки в потоке

    try
    {
        stringstream strStream(fullDateLine);

        IfPlusAddOne(strStream, plusCounter);
        strStream >> year;

        EnsureNextSymbolAndRemove(strStream);
        ...
        if (fullDateLine.size() != 2 + plusCounter + getSize(year) + getSize(month) + getSize(day))
        {
            throw domain_error("");
        }
        else
